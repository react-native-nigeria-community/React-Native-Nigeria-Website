// src/components/commons/EventDetailsTab.jsx
import React, { useState, useMemo } from "react";
import TypographyComponent from "./typography.jsx";
import ImageModal from "./ImageModal.jsx";

const EventDetailsTab = ({ event, getEmbedUrl }) => {
  ...
  // THIS IS WHAT HE ASKED FOR: "Move tabs to constants"
  // useMemo = the correct React way to make it constant + only recalculate when needed
  const tabs = useMemo(() => [
    { id: "speakers", name: "Speakers", count: event.speakers?.length ?? 0 },
    { id: "photos",   name: "Photos",   count: event.images?.length ?? 0 },
    { id: "videos",   name: "Videos",   count: event.videos?.length ?? 0 },
    { id: "gallery",  name: "Gallery",  count: event.embed || event.galleryLink ? 1 : 0 },
  ].filter((tab) => tab.count > 0), [event]);
      ...
      <div className="border-b border-gray-700">
        <nav className="-mb-px flex space-x-6 sm:space-x-10 overflow-x-auto">
          ...
          {tabs.map((tab) => (
            ...
            >
              {tab.name}{" "}
              {tab.count > 1 && <span className="hidden sm:inline">({tab.count})</span>}
            </button>
          ))}
        </nav>
      </div>
        ...
        {activeTab === "videos" && event.videos?.length > 0 && (
          <div className="space-y-8">
            {event.videos.map((url, index) => (
              <div key={index} className="rounded-xl overflow-hidden shadow-2xl bg-black border-2 border-gray-700">
                <iframe
                  src={getEmbedUrl(url)}
                  title={`Video ${index + 1}`}
